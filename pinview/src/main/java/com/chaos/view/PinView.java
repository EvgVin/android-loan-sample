...
public class PinView extends AppCompatEditText {
    ...
    public PinView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mPasswordMask = a.getString(R.styleable.PinView_passwordMask);
        if (mPasswordMask == null) mPasswordMask = DEFAULT_PASSWORD_MASK;
    ...
    private void drawText(Canvas canvas, int i) {
        ...
        if (isPasswordInputType(getInputType())) text = getPasswordText(text);
        drawTextAtBox(canvas, paint, text, i);
    }

    private CharSequence getPasswordText(CharSequence currentText) {
        StringBuilder passwordText = new StringBuilder();
        for (int i = 0; i < currentText.length(); i++) {
            passwordText.append(mPasswordMask);
        }
